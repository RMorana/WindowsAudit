...
Foreach ($Target in $Targets){
	...
	$colLoggedEvents = Get-WmiObject -computerName $Target -query ("Select * from Win32_NTLogEvent Where Type='Erreur' and TimeWritten >='" + $WmidtQueryDT + "'")
	#$colLoggedEvents = Get-WmiObject -computerName $Target -query ("Select * from Win32_NTLogEvent Where Type='Error' and TimeWritten >='" + $WmidtQueryDT + "'")
	Write-Output "..Event Log Warnings"
	$WmidtQueryDT = [System.Management.ManagementDateTimeConverter]::ToDmtfDateTime([DateTime]::Now.AddDays(-14))
	$colLoggedEvents = Get-WmiObject -computerName $Target -query ("Select * from Win32_NTLogEvent Where Type='Avertissement' and TimeWritten >='" + $WmidtQueryDT + "'")
	#$colLoggedEvents = Get-WmiObject -computerName $Target -query ("Select * from Win32_NTLogEvent Where Type='Warning' and TimeWritten >='" + $WmidtQueryDT + "'")
	...
	<font face="Arial" size="1"><b><i>Version Econocom by CCA</i></b></font><br>
